...
- `pnpm lan` — build frontend and serve dashboard over HTTP via `codex-web` bin.
  - Env: `CODEX_SWITCHER_WEB_HOST`, `CODEX_SWITCHER_WEB_PORT`.
...
## Rust direct
- `cargo run --manifest-path src-tauri/Cargo.toml --bin codex-web` — run LAN server directly.
