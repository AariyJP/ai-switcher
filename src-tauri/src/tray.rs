...
use crate::{
    ...
    types::{AccountsStore, ToolKind},
};

const TRAY_ID: &str = "ai-switcher-tray";
...
fn build_menu<R: Runtime>(app: &AppHandle<R>, store: &AccountsStore) -> tauri::Result<Menu<R>> {
    ...
    if store.accounts.iter().all(|account| account.tool != ToolKind::Codex) {
        menu.append(
    ...
    menu.append(&MenuItemBuilder::with_id(OPEN_ITEM_ID, "Open AI Switcher").build(app)?)?;
