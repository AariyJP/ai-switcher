...
fn find_processes(tool: ToolKind) -> anyhow::Result<(Vec<u32>, usize)> {
    ...
    #[cfg(unix)]
    {
        ...
        let mut bg_count = 0;
        let process_names = if tool == ToolKind::Codex {
            read_unix_process_names()
        } else {
            HashMap::new()
        };
        ...
        if let Ok(output) = output {
            ...
            for line in stdout.lines() {
                ...
                let is_desktop = if tool == ToolKind::Codex {
                    is_macos_codex_desktop_process(
                        &command,
                        process_names.get(&pid).map(String::as_str),
                    )
                } else {
                    command.contains(patterns.desktop_macos_marker)
                        && !patterns
