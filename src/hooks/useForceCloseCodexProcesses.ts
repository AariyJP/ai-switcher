...
interface UseForceCloseCodexProcessesOptions {
  processCount: number;
  checkProcesses: () => Promise<Record<"codex" | "claude", ProcessInfo | null> | null>;
...
export function useForceCloseCodexProcesses({
  ...
  const forceCloseCodexProcesses = useCallback(async () => {
    try {
      ...
      const remainingCount = latestProcessInfo?.codex?.count ?? 0;
      ...
      if (result.targeted_count === 0) {
        ...
        showToast(
          `Force closed ${processCount} Codex session${processCount === 1 ? "" : "s"}.`
        );
        ...
        showToast(
          `Could not force close ${remainingCount} Codex session${remainingCount === 1 ? "" : "s"}.`,
          true
        );
      }
